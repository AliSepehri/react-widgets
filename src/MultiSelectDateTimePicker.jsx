...
var MultiSelectDateTimePicker = React.createClass({
  ...
  unselectDay(keyDay, days) {
    if(days instanceof Array) {
      return days.some((day) => {
        if(dates.eq(keyDay, day, 'day')){
          return true;
        }
      });
    }else{
      return dates.eq(keyDay, days, 'day');
    }
  },
  ...
  render() {
    let {
      ...
      , tabIndex, value, days,  editFormat, timeFormat
  ...
  _parse(string){
    ...
    invariant(formats.length,
      ...
      'the MultiSelectDateTimePicker is unable to parse `%s` into a dateTime, ' +
